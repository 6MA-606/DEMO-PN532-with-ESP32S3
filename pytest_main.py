...
def test_main(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
...
def test_main_linux(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_main_macos(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
def test_main_host(app: QemuApp, dut: QemuDut) -> None:
